...
$movieCredits = array_values(array_filter($movieCredits, static fn(array $credit): bool => has_media_poster($credit)));
$tvCredits = array_values(array_filter($tvCredits, static fn(array $credit): bool => has_media_poster($credit)));
$productionCredits = array_values(array_filter($productionCredits, static fn(array $credit): bool => has_media_poster($credit)));
$birthday = format_date($actor['birthday'] ?? null);
$profile = tmdb_img($actor['profile_path'] ?? null, 'w500');
?>
<section class="streamhive-actor-hero streamhive-glass rounded-4 overflow-hidden mb-4">
  <div class="streamhive-actor-hero-bg" style="background-image:url('<?= e($profile) ?>')"></div>
  <div class="row g-0 align-items-stretch streamhive-actor-hero-grid">
    <div class="col-lg-3 col-md-4">
      <div class="streamhive-actor-profile-poster">
        <img src="<?= e($profile) ?>" alt="<?= e($actor['name']) ?> profile">
      </div>
    </div>
    <div class="col-lg-9 col-md-8">
      <div class="streamhive-actor-hero-copy">
        <span class="streamhive-v2-kicker"><i class="fa-solid fa-star"></i> Actor profile</span>
        <h1><?= e($actor['name']) ?></h1>
        <div class="streamhive-actor-meta-row">
        ...
        <p class="streamhive-actor-bio"><?= e((string)($actor['biography'] ?? 'Biography coming soon.')) ?></p>
        <div class="streamhive-actor-stat-row">
        ...
        <div class="streamhive-v2-hero-actions streamhive-actor-share-actions mt-3">
    ...
    <div class="streamhive-actor-credit-panel <?= $tabType === 'movie' ? 'active' : '' ?>" :class="{ active: activeTab === '<?= e($tabType) ?>' }" x-show="activeTab === '<?= e($tabType) ?>'" data-actor-panel="<?= e($tabType) ?>">
      <?php if ($items): ?>
        <div class="streamhive-actor-credit-grid" data-actor-grid="<?= e($tabType) ?>" data-per-page="12">
          ...
          ?>
            <article class="streamhive-actor-credit-card <?= $isFuture ? 'streamhive-is-production' : '' ?>" data-actor-credit-item>
              <?php if ($isFuture): ?>
                <div class="streamhive-actor-credit-poster streamhive-actor-credit-poster-disabled" aria-label="<?= e($title) ?> is not released yet">
                  <img src="<?= e($poster) ?>" alt="<?= e($title) ?> poster">
                  <span class="streamhive-actor-credit-production-badge"><i class="fa-solid fa-clock"></i> Coming soon</span>
                </div>
              <?php else: ?>
                <a href="<?= e($href) ?>" class="streamhive-actor-credit-poster streamhive-js-media-link" aria-label="Open <?= e($title) ?>" data-fetch-content="1" data-media="<?= $creditMedia ?>">
                  <img src="<?= e($poster) ?>" alt="<?= e($title) ?> poster">
                  <span class="streamhive-actor-credit-play"><i class="fa-solid fa-play"></i></span>
                </a>
              <?php endif; ?>
              <div class="streamhive-actor-credit-copy">
                <?php if ($isFuture): ?>
                  <span class="streamhive-actor-credit-title streamhive-actor-credit-title-static"><?= e($title) ?></span>
                <?php else: ?>
                  <a href="<?= e($href) ?>" class="streamhive-actor-credit-title streamhive-js-media-link" data-fetch-content="1" data-media="<?= $creditMedia ?>"><?= e($title) ?></a>
                <?php endif; ?>
                <?php if ($prettyDate !== ''): ?><span class="streamhive-actor-credit-date"><i class="fa-solid fa-calendar-days"></i> <?= e($prettyDate) ?></span><?php endif; ?>
                <?php if ($character !== ''): ?><span class="streamhive-actor-credit-character">as <?= e($character) ?></span><?php endif; ?>
              </div>
            </article>
          <?php endforeach; ?>
        </div>
        <div class="streamhive-actor-credit-footer mt-4" data-actor-pagination="<?= e($tabType) ?>"></div>
      <?php else: ?>
